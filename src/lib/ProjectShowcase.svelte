...
{#if index === showcaseIndex}
	<div class="flex flex-col md:flex-row mt-24 items-center absolute">
		<div
			in:fly={{ x: 50 * reverse(), delay: 150 }}
			out:fly={{ x: -50 * reverse() }}
			class="flex flex-row md:flex-col w-[95%] md/lg:w-2/3">
			<div class="flex flex-col w-1/2 md:w-full md:mt-0 mr-8 md:mr-0">
				...
				<h4
					class="text-apex-midnight text-opacity-25 font-bold tracking-widest mt-6 md/lg:mt-12 mb-3">
					CLIENT
				</h4>
				<a
			...
			<div class="flex flex-col w-1/2 md:w-full -mt-1 md:mt-0 ml-8 md:ml-0">
				<h4 class="text-apex-midnight text-opacity-25 font-bold tracking-widest md/lg:mt-12 mb-3">
					BREAKDOWN
				</h4>
		...
		<div class="mt-14 md/lg:mt-0 md/lg:ml-20 lg:ml-32 w-full relative">
			<img
				src={project.image}
				alt={project.project}
				class="object-cover object-left w-full aspect-square sm:aspect-video md:aspect-square rounded-sm"
			...
			{#if project.imageMobile}
				<img
					...
					class:sm:w-48={project.mobileAspect === 'aspect-[9/16]'}
