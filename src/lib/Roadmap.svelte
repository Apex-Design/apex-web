...
<div class="mt-[4.5rem] md:mt-32 md:ml-32 flex flex-col w-full md:w-2/3 pb-64 md:pb-48">
	{#each STEPS as details, index}
		<div
			...
			<p class="text-apex-midnight text-opacity-40 text-[17px] mt-4 leading-relaxed tracking-wide">
				{details.description}
			</p>
			{#if index < STEPS.length - 1}
				<div class="mt-8 mb-12 md:mt-12 md:mb-16 w-2/3 h-px bg-apex-midnight bg-opacity-10" />
			{/if}
		</div>
	{/each}
</div>
